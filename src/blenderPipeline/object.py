...
class Object:
    def __init__(self, obj: bpy.types.Object, class_id: int, class_name: str, spawn_position: tuple[float, float, float] = [0,0,0]):
    ...
    def setPosition(self, position: tuple[float, float, float]):
        try:
            ...
            print(f"Cannot set position - object reference may be invalid: {e}")
            raise
    ...
    def setRotation(self, rotation: tuple[float, float, float]):
        try:
            ...
            print(f"Cannot set rotation - object reference may be invalid: {e}")
            raise
    ...
    def setScale(self, scale: tuple[float, float, float]):
        try:
            ...
            print(f"Cannot set scale - object reference may be invalid: {e}")
            raise
    ...
    def clearPosition(self):
        try:
            ...
            print(f"Cannot clear position - object reference may be invalid: {e}")
            raise
        ...
        if self.shadow is not None:
            try:
                ...
                print(f"Cannot clear shadow position: {e}")
    ...
    def setKeyframe(self, position: tuple[float, float, float], rotation: tuple[float, float, float], scale: tuple[float, float, float], frame: int):
        ...
        if self.shadow is not None:
            try:
                ...
                print(f"Cannot insert shadow keyframes: {e}")
        ...
        try:
            ...
            if self.shadow is not None:
                anim_data = self.shadow.animation_data
                if anim_data:
                    action = anim_data.action
                    if action:
                        # Use index-based iteration to avoid holding fcurve references
            ...
            print(f"Could not set keyframe interpolation: {e}")
    ...
    def configure_shadow(self,point_radius: float = 0.02):
        ...
        try:
            ...
            print(f"Failed to configure shadow geometry nodes: {e}")
        ...
        try:
            ...
            if not has_kiri_modifier:
                if retry_if_missing:
                    ...
                    print(f"Object {self.obj.name} is marked as 3DGS but has no KIRI_3DGS_Render_GN modifier yet")
                    return False
            ...
            if hasattr(obj, 'sna_kiri3dgs_active_object_update_mode'):
                ...
                if current_mode == 'Disable Camera Updates':
                    ...
                    print(f"Enabled camera updates for 3DGS object: {obj.name}")
                elif current_mode == 'Enable Camera Updates':
                    # Already enabled, just confirm
                    print(f"Camera updates already enabled for 3DGS object: {obj.name}")
                return True
            else:
                print(f"3DGS object {obj.name} missing camera update property")
                return False
            ...
            if retry_if_missing:
                return False
            print(f"3DGS object {self.obj.name} modifier not ready yet: {e}")
            return False
        except Exception as e:
            print(f"Failed to configure 3DGS object {self.obj.name}: {e}")
            return False
